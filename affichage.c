...
void affichage_map(map_objt tab[COL][LIG],SDL_Surface *screen, int zoom, image image, SDL_Rect coord_init)
{
	...
	rcCase.x = coord_init.x;
	rcCase.y = coord_init.y;
	rcCase.h = 16*zoom;
	rcCase.w = 16*zoom;
	...
	for(col=0;col<COL;col++)
	  {
	    rcCaseDest.y = 16*zoom*col;
	    for(lig=0;lig<LIG;lig++)
	      {
            rcCaseDest.x = 16*zoom*lig;
...
void mise_image_struct_tableau(map_objt tab[COL][LIG], image image)
{
    ...
    for (col=0;col<COL;col++)
        for (lig=0;lig<LIG;lig++)
            {
                tab[col][lig].s = SDL_LoadBMP ("image/herbe.bmp");
                if (tab[col][lig].a==1)
                    tab[col][lig].s = SDL_LoadBMP ("image/chene.bmp");
                if (tab[col][lig].a==2)
                    tab[col][lig].s = SDL_LoadBMP ("image/salade.bmp");
                if (tab[col][lig].a==3)
                    tab[col][lig].s = SDL_LoadBMP ("image/fraisier.bmp");
                if (tab[col][lig].a==4)
                    tab[col][lig].s = SDL_LoadBMP ("image/ble.bmp");
...
image image_init()
{
  ...
  image.salade = SDL_LoadBMP ("image/salade.bmp");
...
image zoom_image(image image, float zoom)
{
  ...
  image.salade = rotozoomSurface(image.salade, 0, zoom, 1);
