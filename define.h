/*************************/
/* define.h              */
/* auteur : Bacp Clement */
/* et Calba Antonin      */
/*************************/

#define COL 100
#define LIG 100
#define taille 32

#define NB_Perso 10

#define pousse_fraise 1
#define fraise 2

#define pousse_coton 3
#define coton 4

#define pousse_ble 5
#define bles 6

#define pousse_bois 7
#define bois 8


#define annuler 0
// Condition: les actions selectionnable ne doivent pas être multiple de 10
#define action_couper 11
#define action_couper_bois 11
#define action_couper_plante 12
#define action_planter_fraise 13
#define action_planter_coton 14
#define action_planter_ble 15
#define action_planter_bois 16
#define action_miner 99
#define action_manger 10000
#define action_ramasser 97
#define action_deposer 98
#define action_stockpile 28
#define action_fabriquer 21 // a separer selon les besoins
